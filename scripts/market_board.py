...
# ── WHICH SESSION A QUOTE BELONGS TO ─────────────────────────────────────
# Distinct from grade_calls.board_session, deliberately. That one answers "has
# a new CLOSE landed yet", and is conservative on purpose: a Tuesday pre-open
# issue holds no Tuesday settle, so it reports Monday and the grader waits.
#
# These two answer a different question: the number PRINTED on the board is
# yfinance's last_price, and the change beside it is against the source's
# previous_close. So:
#     quote_session      the session the printed number belongs to
#     prev_close_session the session the change is measured against
#
# Tuesday 06:00 CT  -> quote Tuesday (in progress), prev Monday's settle
# Friday  17:04 CT  -> quote Friday  (settled),     prev Thursday's settle
# Saturday/Sunday   -> quote Friday  (last session), prev Thursday's settle
# One rule covers all three: the fetch date if it traded, else the last session.

def quote_session(fetched_iso):
    """The session the board's printed numbers belong to. None if unparseable."""
    from contract_calendar import is_trading_day, prior_trading_day
    try:
    ...
    try:
        ...
        dt = dt.astimezone(ZoneInfo("America/Chicago")) if dt.tzinfo else dt
    except Exception:
        pass
    d = dt.date()
    return d if is_trading_day(d) else prior_trading_day(d)


def prev_close_session(fetched_iso):
    """The session the board's CHANGE column is measured against."""
    from contract_calendar import prior_trading_day
    qs = quote_session(fetched_iso)
    return prior_trading_day(qs) if qs else None


def locked_row(daily, key):
    """The {prev, pct} locked for `key` at generation, or None."""
    row = ((daily or {}).get("locked_changes") or {}).get(key)
    return row if isinstance(row, dict) else None


def locked_change(daily, key):
    """The change for `key` as it was LOCKED at generation, or None.

    v5.4: generate_daily now copies each instrument's previous close and
    percent change out of prices.json when it locks the board. That number is
    the source's own close-over-close move, computed at fetch time. Walking the
    archive for it instead compares two SNAPSHOTS taken at different times of
    day and quietly mixes in overnight drift: on 2026-09-12 the archive walk
    gave corn -0.49% and beans -1.39% where the fetch had -0.63% and -1.81%.

    Returns None for the 185 issues archived before the lock existed; callers
    fall back to the walk for those."""
    row = locked_row(daily, key)
    if row is None:
        return None
    pct = row.get("pct")
...
def prior_board(daily, archive_dir="data/daily-archive"):
    ...
    When the board is empty, session_date says WHY. None means no earlier
    archived issue exists at all. "~too-far:<stem>" means one exists and was
    deliberately refused for being more than a session back; <stem> names it,
    so the log can point at the file instead of implying there is none.
    Callers that only want a board can ignore it -- the board is empty either
    way -- but anything that reports a reason must tell the two apart.
    """
        ...
        for f in reversed(files):
            ...
            if lp and not same_board(lp, cur):
                if _too_far(issue, daily or {}, stem):
                    ...
                    #
                    # SAY WHICH OF THE TWO SILENCES THIS IS. Returning a bare
                    # empty board here made "we found nothing" and "we found
                    # one and it was too far" arrive identically, and the gate
                    # then logged "no earlier archived issue found" on 15
                    # weekend issues where an earlier issue plainly exists. A
                    # reason that is wrong is worse than no reason: it sends
                    # whoever reads the log looking for a missing file.
                    return {}, "~too-far:" + stem, None
...
def _selftest():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        lp, day, err = prior_board(issues["2026-09-13"], tmp)
        chk(lp == {} and err is None,
            "with Thursday's board overwritten, Sunday shows NO change column "
            "rather than a two-session move beside one-session prose")
        # AND IT SAYS WHICH SILENCE THIS IS. An empty board that cannot name
        # its reason made the gate log "no earlier archived issue found" on 15
        # weekend issues where the earlier issue is right there in the folder.
        chk(day == "~too-far:2026-09-10",
            "and it names the issue it refused, so the reason it gives is the "
            "real one rather than 'nothing found'")
